...
class FKVWindowAttention_kv(nn.Module):

    def __init__(self, dim, window_size,v_dim, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
...
#         self.pool = nn.AdaptiveAvgPool2d(7)
        self.sr = PSA_p(dim, dim, kernel_size=1, stride=1)
    ...
    def forward(self, skip, x_up, mask=None):
        ...
        # kv = self.kv(skip)
        q = skip
        kv_ = x_up.permute(0, 2, 1).reshape(B_, C, 7, 7)
        kv_ = self.sr(kv_).reshape(B_, C, -1).permute(0, 2, 1)
        kv_ = self.norm(kv_)
        kv_ = self.act(kv_)
        kv = self.kv(kv_).reshape(B_, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        ...
        q = q * self.scale
        if self.v2:
            denom = torch.norm(q, dim=-1, keepdim=True) @ torch.norm(k, dim=-1, keepdim=True).transpose(-2, -1)
            attn = (q @ k.transpose(-2, -1))/ denom.clamp(min=1e-6)
...
class FKVSWINBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, v_dim, window_size=7, shift_size=0,
        ...
        self.attn = FKVWindowAttention_kv(
...
class BasicFKVSWINLayer(nn.Module):
    """ A basic FKVSWIN layer for one stage.
...
class FKVSWIN(nn.Module):
    def __init__(self,
        ...
        self.crf_layer = BasicFKVSWINLayer(
                dim=embed_dim,
                depth=depth,
                num_heads=num_heads,
                v_dim=v_dim,
...
if __name__ == '__main__':
    model = FKVSWIN()
    x1 = torch.rand(2, 1536, 22, 44)
    x2 = torch.rand(2, 512, 22, 44)
    outs = model(x1,x2)
