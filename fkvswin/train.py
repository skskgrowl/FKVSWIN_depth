...
from networks.FkvSwinDepth import FkvSwinDepth


parser = argparse.ArgumentParser(description='FkvSwin PyTorch implementation.', fromfile_prefix_chars='@')
parser.convert_arg_line_to_args = convert_arg_line_to_args

parser.add_argument('--mode',                      type=str,   help='train or test', default='train')
parser.add_argument('--model_name',                type=str,   help='model name', default='fkvswin')
...
parser.add_argument('--save_freq',                 type=int,   help='Checkpoint saving frequency in global steps', default=5790)
...
parser.add_argument('--variance_focus',            type=float, help='lambda in paper: [0, 1], higher value more focus on minimizing variance of error', default=0.5)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = FkvSwinDepth(version=args.encoder, inv_depth=False, max_depth=args.max_depth, pretrained=args.pretrain)
    model.train()
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
                print('[epoch][s/s_per_e/gs]: [{}][{}/{}/{}], lr: {:.12f}, loss: {:.12f}, avg_loss: {:.12f}'.format(epoch, step, steps_per_epoch, global_step, current_lr, loss, avg_loss))
            ...
            if global_step and global_step % args.log_freq == 0 and not model_just_loaded:
                ...
                print(print_string.format(args.gpu, examples_per_sec, avg_loss, var_sum.item(), var_sum.item()/var_cnt, time_sofar, training_time_left))
...
def main():
    ...
    if save_files:
        ...
        command = 'cp fkvswin/train.py ' + aux_out_path
        os.system(command)
        command = 'mkdir -p ' + networks_savepath + ' && cp fkvswin/networks/*.py ' + networks_savepath
        os.system(command)
        command = 'mkdir -p ' + dataloaders_savepath + ' && cp fkvswin/dataloaders/*.py ' + dataloaders_savepath
        os.system(command)
